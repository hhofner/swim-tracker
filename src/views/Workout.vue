...
const workout = workouts[routeId - 1]
const commentValue = ref('')
const workoutLogs = useStorage<WorkoutLog[]>('workout-logs', [])
    ...
    <table class="w-full max-w-2xl text-left mb-4 border border-gray-300 dark:border-gray-700 rounded-sm">
      ...
      <thead>
        <tr class="bg-primary text-white dark:bg-primary-800">
          <th scope="col" class="p-3 border-b border-gray-300 dark:border-gray-700">Exercise</th>
          <th scope="col" class="p-3 border-b border-gray-300 dark:border-gray-700">Amount</th>
        </tr>
      </thead>
      <tbody>
        <template v-for="subsection in workout.subSections">
          <tr v-if="subsection.title" class="bg-secondary/30 dark:bg-secondary-800/30">
            <td colspan="2" class="p-2 font-semibold border-b border-gray-300 dark:border-gray-700">
              <span>{{ subsection.title }}</span>
              <span v-if="subsection.multiplier" class="ml-4 text-sm text-stone-500 dark:text-stone-400">{{ subsection.multiplier }} times
                through</span>
            </td>
          </tr>
          <tr v-for="exercise in subsection.workouts" class="odd:bg-white even:bg-gray-100 dark:odd:bg-gray-900 dark:even:bg-gray-800">
            <td class="p-2 border-b border-gray-300 dark:border-gray-700 dark:text-gray-200">{{ exercise.title }}</td>
            <td class="p-2 border-b border-gray-300 dark:border-gray-700 dark:text-gray-200 text-right">{{ exercise.total }}</td>
          </tr>
        </template>
      </tbody>
      <tfoot>
        <tr class="bg-primary/30 dark:bg-primary-800/30">
          <th scope="row" class="p-4 border-t border-gray-300 dark:border-gray-700 dark:text-gray-200">Total</th>
          <td class="p-4 border-t border-gray-300 dark:border-gray-700 dark:text-gray-200 text-right">{{ workout.total }}</td>
        </tr>
      </tfoot>
    </table>
    <div class="space-x-2">
      <UButton size="xs" :variant="favoriteWorkouts.includes(workout.id) ? 'solid' : 'soft'" icon="i-lucide-star"
        @click.stop="handleFavorite">
        Favorite
      </UButton>
      <UButton size="xs" :variant="toSwimWorkouts.includes(workout.id) ? 'solid' : 'soft'"
        icon="i-lucide-bookmark-check" @click.stop="handleBookmark">
        To Swim
      </UButton>
    </div>
